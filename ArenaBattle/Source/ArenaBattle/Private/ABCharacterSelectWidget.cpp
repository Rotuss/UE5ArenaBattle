...
{
    true == bForward ? ++CurrentIndex : --CurrentIndex;
    ...
    auto Asset = ABGameInstance->StreamableManager.LoadSynchronous<USkeletalMesh>(AssetRef);
    if (nullptr != Asset)
    {
...
void UABCharacterSelectWidget::NativeConstruct()
{
    ...
    TextBox = Cast<UEditableTextBox>(GetWidgetFromName(TEXT("editPlayerName")));
...
void UABCharacterSelectWidget::OnConfirmClicked()
{
    ...
    if (0 >= CharacterName.Len() || 10 < CharacterName.Len())
    {
        return;
    }
